...
- Once the `/search?q=` page exists (TODO §5), the trigger becomes a real `<a href>` that JS intercepts (Ogygia's is one): shareable queries, middle-click/new-tab, and a working fallback when the modal chunk fails to load. SSR'd no-JS results stay impossible on static hosting — say so, don't imitate.
...
`buildEnd` never fires in dev, so dev uses the cheaper core-style path: markdown-renderer-based indexing with per-file re-index on HMR and version-busted middleware serving of the same artifact shapes. Documented fidelity gap: vite-transform/dynamic-route output is only in production indexes — and Vue components are unrendered tags in dev, so the skip-list extractor indexes their raw slot text where prod indexes rendered output: dev and prod index _different text_ on component-heavy pages. Nothing is silently dropped, though — the allowlist alternative (slimsearch's `CONTENT_BLOCK_TAGS` + `preserveTags` option) loses unlisted components entirely.
