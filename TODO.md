...
- [ ] `/search?q=` page (vuejs/vitepress#2983) + OpenSearch descriptor (#2855)
...
- [ ] Upstream `provider: 'custom'` / search slot in vitepress core to retire the alias hijack — DESIGN §12.1
...
- [ ] Recent searches / favorites (DocSearch parity; never scoped)

## 6. Minisearch provider

- [ ] Benchmarks for the new local search: index size / query latency / memory vs core local search and Pagefind, on the docs corpus and a synthetic large site (research baselines are in DESIGN §11 and session notes)
...
- [ ] Dev-mode fidelity gap (markdown-renderer path lacks vite transforms/dynamic routes) — documented; improve if cheap
...
- [ ] zbsearch — re-check ~Sep 2026 (best CJK ergonomics; plugin packages currently uninstallable due to `workspace:*` publish bug)
